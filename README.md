...
Derivative of a spherical harmonic tensor with respect to orignal coordinate system using wignerD rotation as the limit of the change in the tensor divided by the $\theta$ and $\phi$ that approaches zero. 

    equivalentFeatures.Derivative_direction(Y, L_max,nx, ny, nz)

This function  describes the infinitesimal change of a spherical harmonic tensor when the object itself is rotated about a specified axis. For a unit axis $\mathbf{n}$ and a small rotation angle $\epsilon$ following the right-hand rule, it is defined as
$\frac{dY}{d\epsilon}=\lim_{\epsilon\to 0}\frac{D(R_{\mathbf{n}}(\epsilon))Y-Y}{\epsilon}=-i(\mathbf{n}\cdot\mathbf{J})Y,\$
where $D(R_{\mathbf{n}}(\epsilon))$ is the Wigner $D$ matrix and $\mathbf{J}$ denotes the angular momentum generators.
     ...
     Y = ComplexF64[
     0,
     -0.3722226076794696 - 0.271399288608001im,
     0.5591795398121944,
     0.3722226076794696 - 0.271399288608001im,
     0.04653914995395994 + 0.16070433622107375im,
     -0.47952498842074204 - 0.3426684736230381im,
     -0.011042843185922649,
     0.4795249884207421 - 0.3426684736230381im,
     0.04653914995395998 - 0.16070433622107372im
     ]
     x, y, z = 2.12, -4.32, 2.11
     ε=1e-4
    # --- Normalize direction ---
    norm_ = sqrt(x^2 + y^2 + z^2)
    x /= norm_; y /= norm_; z /= norm_
    # --- Convert to spherical angles ---
    theta = acos(z)
    phi   = atan(y, x)
    # --- Build Wigner-D for alignment ---
    R1 = wignerD(1, 0.0, -theta, -phi)
    R2 = wignerD(2, 0.0, -theta, -phi)
    Y_delta = copy(Y);
    # Step 1: rotate into local frame
    Y_delta[2:4] = R1 * Y_delta[2:4]
    Y_delta[5:9] = R2 * Y_delta[5:9]
    # Step 2: small rotation around LOCAL z-axis
    Y_delta[2:4] = wignerD(1, 0.0, 0.0, ε) * Y_delta[2:4]
    Y_delta[5:9] = wignerD(2, 0.0, 0.0, ε) * Y_delta[5:9]
    # Step 3: rotate back to global frame
    Y_delta[2:4] = R1' * Y_delta[2:4]
    Y_delta[5:9] = R2' * Y_delta[5:9] 
    # finite difference
    fd = (Y_delta .- Y) ./ ε
    dY=equivalentFeatures.Derivative_direction(Y,3,x,y,z);
    sum(abs.(fd - dY));
	
 The directional derivative of a spherical harmonic tensor with respect to a rotation axis $\mathbf{n}$ represents the infinitesimal change of the tensor under a small rotation about that axis. Specifically, it is defined as the limit of the change in the tensor divided by the rotation angle as the angle approaches zero. This captures how the representation varies under infinitesimal rotations and is generated by the angular momentum operators.
...
Reference vectors are extracted from input spherical harmonic tensors (V_input) via the ReferencesExtract or SelfProductMatrix function. The spherical harmonic tensor (V_output) is converted into a rotation-invariant vector, which is insensitive to the rotation of (V_input) and (V_output), using the W3jProductCompact function and the reference vectors. This rotation-invariant vector is then recovered as a spherical harmonic tensor with coordinate information using an auxiliary matrix generated by the DecodeMatrixCompact function and the reference vectors.
 ...
 V1 and V2 are spherical harmonic tensors to be encoded,  one shell of V1 and one shell of V2 will be Clebsch–Gordan (CG) producted to generate a set of spherical harmonic tensor, the maximum size of each set is n, the return is a list for the norm of each spherical harmonic tensors and pairwise products among spherical harmonic tensors from these sets.
e.g.
